...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header("Location: absensi-guru-pengajar.php");
    exit;
}
...
if (!$status || empty($nip_list)) {
    echo "<script>alert('Data tidak lengkap!'); window.location='absensi-guru-pengajar.php';</script>";
    exit;
}
...
foreach ($nip_list as $nip) {
    ...
    if (mysqli_num_rows($check) > 0) {
        echo "<script>
                ...
                window.location='absensi-guru-pengajar.php';
              </script>";
        exit;
    }
...
header('Location: absensi-guru-pengajar.php?status=added');
exit;
