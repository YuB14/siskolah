...
if (!isset($_GET['id_keuangan'])) {
    header('Location: pemasukan-pengeluaran-bendahara.php');
    exit;
}
...
if (mysqli_num_rows($cek) === 0) {
    ...
    header('Location: pemasukan-pengeluaran-bendahara.php?status=notfound');
    exit;
}
...
if ($query) {
    header('Location: pemasukan-pengeluaran-bendahara.php?status=deleted');
    exit;
} else {
    // Jika gagal, arahkan dengan status gagal
    header('Location: pemasukan-pengeluaran-bendahara.php?status=error');
    exit;
}
?>
