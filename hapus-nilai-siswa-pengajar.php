...
if (!isset($_GET['id_nilai'])) {
    ...
    header('Location: nilai-siswa-pengajar.php?status=error');
    exit;
}
...
if (mysqli_num_rows($cek) > 0) {
    ...
    if ($hapus) {
        ...
        header('Location: nilai-siswa-pengajar.php?status=deleted');
        exit;
    } else {
        // Jika gagal menghapus (misal karena foreign key constraint)
        header('Location: nilai-siswa-pengajar.php?status=delete_failed');
        exit;
    }
} else {
    // Jika data tidak ditemukan
    header('Location: nilai-siswa-pengajar.php?status=not_found');
    exit;
}
?>
