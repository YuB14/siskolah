...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($tanggapan) || empty($status) || empty($tanggal_tanggapan)) {
        echo "<script>alert('Semua field wajib diisi!'); window.location='pengaduan-guru-pengajar.php';</script>";
        exit;
    }
    ...
    if (mysqli_query($koneksi, $query)) {
        echo header('Location: pengaduan-guru-pengajar.php?status=added');
    } else {
        echo "<script>
                alert('Gagal menyimpan tanggapan! Coba lagi.');
                window.location.href = 'pengaduan-guru-pengajar.php';
              </script>";
    }
} else {
