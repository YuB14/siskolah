...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($tanggapan) || empty($tanggal_tanggapan)) {
        echo "<script>alert('Semua field wajib diisi!'); window.location='tanggapan-kritik-saran-pengajar.php';</script>";
        exit;
    }
    ...
    if (mysqli_query($koneksi, $query)) {
        echo header('Location: tanggapan-kritik-saran-pengajar.php?status=added');
    } else {
        echo "<script>
                alert('Gagal menyimpan tanggapan! Coba lagi.');
                window.location.href = 'tanggapan-kritik-saran-pengajar.php';
              </script>";
    }
} else {
