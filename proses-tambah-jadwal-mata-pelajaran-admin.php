...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header("Location: tambah-jadwal-mata-pelajaran-admin.php");
    exit;
}
...
// ===== Cek bentrok jadwal untuk guru (hari + nip) =====
$cekGuruSql = "
    SELECT id_jadwal, jam_mulai, jam_selesai
    FROM jadwal_mapel
    WHERE hari = ?
...
if ($stmtG = $koneksi->prepare($cekGuruSql)) {
    $stmtG->bind_param(
        'ssssssssss',
        $hari,
        $nip,
    ...
    if (!$stmtG->execute()) {
        $stmtG->close();
        js_alert_back("Gagal memeriksa bentrok guru: " . $koneksi->error);
    }

    $resG = $stmtG->get_result();
    if ($resG && $resG->num_rows > 0) {
        $rowG = $resG->fetch_assoc();
        $existing_start = $rowG['jam_mulai'];
        $existing_end = $rowG['jam_selesai'];
        $stmtG->close();
        js_alert_back("Guru tersebut sudah mengajar pada jam $existing_start - $existing_end di hari yang sama.");
    }

    $stmtG->close();
} else {
    js_alert_back("Gagal menyiapkan query bentrok guru: " . $koneksi->error);
}
...
if ($stmt2 = $koneksi->prepare($insertSql)) {
    ...
    if ($stmt2->execute()) {
        ...
        header("Location: jadwal-mata-pelajaran-admin.php?status=added");
        exit;
    } else {
