...
if (empty($nisn) || empty($tanggal) || empty($jenis) || empty($isi)) {
    echo "<script>alert('Semua field wajib diisi!'); window.location='tambah-kritik-saran-siswa.php';</script>";
    exit;
}
...
if (mysqli_num_rows($cekSiswa) == 0) {
    echo "<script>alert('NISN tidak ditemukan dalam data siswa!'); window.location='tambah-kritik-saran-siswa.php';</script>";
    exit;
}
...
if (mysqli_query($koneksi, $sql)) {
    echo header('Location: kritik-saran-siswa.php?status=added');
} else {
    echo "<script>
            alert('Terjadi kesalahan saat menyimpan data: " . mysqli_error($koneksi) . "');
            window.location='tambah-kritik-saran-siswa.php';
          </script>";
}
?>
