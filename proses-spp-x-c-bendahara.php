...
if (isset($_POST['simpan'])) {
    ...
    // Validasi input
    if (empty($nisn_array)) {
        header("Location: spp-x-c-bendahara.php?status=error&msg=Pilih minimal 1 siswa");
        exit();
    }
    
    if (empty($bulan_array)) {
        header("Location: spp-x-c-bendahara.php?status=error&msg=Pilih minimal 1 bulan");
        exit();
    }
    ...
    try {
        ...
        header("Location: spp-x-c-bendahara.php?status=added&msg=" . urlencode($message));
        exit();
        ...
        header("Location: spp-x-c-bendahara.php?status=error&msg=" . urlencode("Terjadi kesalahan: " . $e->getMessage()));
        exit();
    }
    
} else {
    // Jika akses langsung tanpa submit
    header("Location: spp-x-c-bendahara.php");
    exit();
}
?>